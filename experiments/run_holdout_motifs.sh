...
SRC_DIR="paper2/data/raw/holdout_graphs"
OUT_DIR="paper2/data/raw/holdout_runs"
...
for atg in "$SRC_DIR"/*.json; do
  ...
  node tools/search_leader.js \
    --atg "$atg" \
    --objective balanced \
    --out "$OUT_DIR/$name/leader_search.json" \
    --csv "$OUT_DIR/$name/leader_search.csv" >/dev/null
done

echo "Done. Hold-out raw runs saved to $OUT_DIR"
